    ...
    <div>
      <div style={{width: width, height: height}} className={` ${absolute ? 'absolute' : 'relative'} ${customClass}  group flex items-end  cursor-pointer`} onClick={toggleGallery}>
          <Image
            src={imageSrc}
            alt={imageAlt}
            sizes={`(max-width: 768px) 400px, (max-width: 1200px) 85vw, ${width ? width : '570px'}`}
            fill
            className={`${rounded ? 'rounded-tr-[55px]' : ''} object-cover `}
          />
          ...
          <div className={`${titleColor === '#FFFFFF' ? 'bg-black/30' : 'bg-white/30'} group-hover:px-4 duration-500 flex w-fit z-10 ml-6 mb-4 rounded-xl px-2 py-1 md:mr-20`}>
            <h2 style={{color: titleColor}} className='relative z-10 md:max-w-[380px] text-base duration-500 '>{title}</h2>
          </div>
      </div>
