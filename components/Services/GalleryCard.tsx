...
const GalleryCard = ({width, height, title, titleColor, expandColor, imageSrc, imageAlt, rounded, absolute, gallery}:GalleryCardTypes) => {
  ...
  return (
    <div>
      <div style={{width: width, height: height}} className={`${absolute ? 'absolute' : 'relative'} group flex items-end  cursor-pointer`} onClick={toggleGallery}>
          <Image src={imageSrc} alt={imageAlt} fill className={`${rounded ? 'rounded-tr-[55px]' : ''} object-cover duration-300 group-hover:scale-110 `} />
          ...
          <h2 style={{color: titleColor}} className='relative shadow-lg z-10 text-lg duration-300 group-hover:opacity-0 pl-6 pb-4'>{title}</h2>
      </div>
